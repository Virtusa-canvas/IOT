...
uint32_t targetTime = 50;       // for next 1 second timeout
...
void loop() {
  ...
  if (targetTime < millis()) {
    ...
    //  tft.setTextColor(0x39C4, TFT_BLACK);  // Leave a 7 segment ghost image, comment out next line!
      ...
      tft.setTextColor(0xF800); // Orange
      omm = mm;

      if (hh<10) xpos+= tft.drawChar('0',xpos,ypos);
...
tft.drawString(s,0,64,4); // Right justified string drawing to x position 75
      String scolour = String(colour,'80%');
      scolour.toUpperCase();
      char buffer[20];
